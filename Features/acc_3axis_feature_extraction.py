...
def compute_acc_features_for_repetition(rep_df, sampling_rate, body_part):
    """
    Compute 3-axis accelerometer features for one repetition.
    The body part is integrated in the feature names.

    Parameters:
      rep_df : DataFrame
          DataFrame with accelerometer data columns 'X', 'Y', 'Z' for one repetition.
    ...
    Returns:
      feats : dict
          Dictionary with features named like:
            X_Shoulder_acc_Mean, X_Shoulder_acc_Std, ...,
            Magnitude_Shoulder_acc_Mean, etc.
    """
    feats = {}
    channels = ['X', 'Y', 'Z']
    ...
    for ch in channels:
        key_prefix = f"{ch}_{body_part}_acc"
    ...
    mag_prefix = f"Magnitude_{body_part}_acc"
    ...
    # Optionally, add the repetition duration (in seconds) as a feature.
    feats[f"RepetitionDuration_{body_part}_acc"] = rep_df.shape[0] / sampling_rate

    return feats


def extract_acc_3axis_features_from_file(csv_path, body_part, sampling_rate=100):
    """
    Reads a CSV file containing IMU accelerometer data (expected columns: X, Y, Z, Repetition)
    and computes 3-Axis Acceleration features for each repetition.
    ...
    Returns:
      df_feats : DataFrame
          ...
          X_Shoulder_acc_Mean, Magnitude_Shoulder_acc_Energy, etc.
    """
    ...
    for rep_id, rep_data in df.groupby("Repetition"):
        feats_dict = compute_acc_features_for_repetition(rep_data, sampling_rate, body_part)
...
def extract_all_acc_3axis_features(
        base_directory="processed_data_35_i",
        body_parts=['Shoulder', 'Forearm', 'Pelvis', 'Upperarm', 'Torso', 'Palm'],
        sampling_rate=100,
        output_csv="Features/Extracted/acc_3axis_features_IMU.csv",
        metadata_file=None
):
    """
    Iterates over each body part (processing only the 'acc' folder), reads
    all accelerometer CSV files (with columns X, Y, Z, Repetition),
    computes 3-Axis Acceleration features (with integrated body part in the feature names)
    ...
    Parameters:
      ...
      metadata_file : str or None
          Optional CSV file with subject metadata to merge based on "Subject".
    """
    all_dfs = []

    for bp in body_parts:
        sensor_folder = os.path.join(base_directory, bp, "acc")
        ...
        for csv_path in csv_files:
            ...
            try:
                df_feats = extract_acc_3axis_features_from_file(csv_path, bp, sampling_rate)
                all_dfs.append(df_feats)
            except Exception as e:
                print(f"[ERROR] Processing {csv_path} failed: {e}")

    if not all_dfs:
        print("[INFO] No 3-Axis Acceleration features extracted. Check your data paths.")
        return
    ...
    print(f"[INFO] 3-Axis Acceleration features saved to {output_csv}")
